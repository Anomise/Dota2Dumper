...
static const char* ReadAsciiField(const void* base, int offset) {
...
bool Dumper::TryDumpClass(void* scope, const char* name) {
    ...
    if (!SEH_IsReadable(binding, 0x80)) return false;

    const char* className = ReadAsciiField(binding, g_BindingLayout.name_offset);
    ...
    int32_t classSize = 0;
    if (g_BindingLayout.size_offset >= 0)
    ...
    if (g_BindingLayout.parent_offset >= 0) {
        ...
        if (SEH_ReadPtr(binding, g_BindingLayout.parent_offset, &parentBinding)) {
            if (parentBinding && parentBinding != binding && SEH_IsReadable(parentBinding, 0x20)) {
                const char* pn = ReadAsciiField(parentBinding, g_BindingLayout.name_offset);
                if (pn) parentName = pn;
            }
        }
    }
    ...
    for (int si = 0; si < 6; si++) {
        ...
        for (int i = 0; i < fieldCount && i < 50; i++) {
            void* fa = (void*)((uintptr_t)fieldsPtr + i * stride);
            if (!SEH_IsReadable(fa, stride)) break;
            const char* fn = ReadAsciiField(fa, 0x00);
            if (!fn) continue;
            int32_t fo = 0;
            if (!SEH_ReadI32(fa, 0x10, &fo)) continue;
            if (fo < 0 || fo > 0x100000) continue;
            valid++;
        }
        if (valid > bestCount) {
            bestCount = valid;
    ...
    for (int i = 0; i < fieldCount; i++) {
        void* fieldAddr = (void*)((uintptr_t)fieldsPtr + i * bestStride);
        if (!SEH_IsReadable(fieldAddr, bestStride)) continue;

        const char* fn = ReadAsciiField(fieldAddr, 0x00);
        if (!fn) continue;
        ...
        if (SEH_ReadPtr(fieldAddr, 0x08, &typePtr)) {
            if (typePtr && SEH_IsReadable(typePtr, 0x10)) {
                const char* tn = ReadAsciiField(typePtr, 0x08);
                if (tn) typeName = tn;
            }
        }
